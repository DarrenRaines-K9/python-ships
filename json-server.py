...
class JSONServer(HandleRequests):
    ...
    def do_POST(self):
        ...
        if url["requested_resource"] == "ship":
            new_item = create_ship(
                request_body
            )  # Function to be written in ship_view.py
        elif url["requested_resource"] == "dock":
            new_item = create_dock(
                request_body
            )  # Function to be written in dock_view.py
        elif url["requested_resource"] == "hauler":
            new_item = create_hauler(
                request_body
            )  # Function to be written in hauler_view.py
        else:
            return self.response(
                "Invalid resource",
                status.HTTP_404_CLIENT_ERROR_RESOURCE_NOT_FOUND.value,
